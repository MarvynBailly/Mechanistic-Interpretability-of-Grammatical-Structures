...
## Typo IOI ("Blurry Search")
Does the IOI circuit still work when a name is misspelled? A human reads "When Mary and John went to the store, Jhon gave a drink to" and still answers Mary. We test whether GPT-2 small's name-matching heads (Duplicate Token, Induction, S-Inhibition) do the same kind of fuzzy match, or only match exact tokens.

### Data
Generated by [`data_generation/generate_typo_ioi_pairs.py`](data_generation/generate_typo_ioi_pairs.py). Every condition uses the same 492 base examples (IO, S, template, object), so only the typo changes:

| Condition | Example (typo at S2) |
|-----------|----------------------|
| exact | When Christopher and Susan went to the store, Susan gave the package to |
| swap | ..., Susna gave the package to |
| drop | ..., Susn gave the package to |
| double | ..., Susann gave the package to |
| sub (keyboard neighbour) | ..., Sysan gave the package to |
| unrelated (control) | ..., Mark gave the package to |

We run both `typo_target="s2"` (the repeated subject is misspelled) and `typo_target="s1"` (the first mention is misspelled). Typo'd names are usually 2-3 BPE tokens while the original name is one token. To keep path patching valid, each corrupt prompt uses random names where the typo'd slot gets the same kind of typo and the same token length, so clean and corrupt prompts are position-aligned.

### Method
[`general_analysis/typo_ioi_analysis.py`](general_analysis/typo_ioi_analysis.py) runs, for each condition:
- behaviour: logit diff `logit(IO) - logit(S)` and % of examples with IO > S
- the same head → logits path patching used above
- the characteristic attention of each known IOI head class (Wang et al.): S2→S1 for duplicate token heads, S2→token after S1 for induction heads, END→S2 for S-inhibition heads, END→IO for name movers

Prompts differ in length, so logits and attention are read at each example's own END position.

```powershell
py .\data_generation\generate_typo_ioi_pairs.py
$env:PYTHONIOENCODING="utf-8"; py .\general_analysis\typo_ioi_analysis.py --target s2 --n 300
$env:PYTHONIOENCODING="utf-8"; py .\general_analysis\typo_ioi_analysis.py --target s1 --n 300
```

### Results (GPT-2 small, n=300 per condition)

| Condition | Logit diff (typo S2) | IO>S (S2) | Logit diff (typo S1) | IO>S (S1) | Effect map r vs exact (S2 / S1) |
|-----------|------|------|------|------|------|
| exact | 3.71 | 99% | 3.71 | 99% | 1.00 / 1.00 |
| swap | 1.05 | 85% | 2.19 | 99% | 0.77 / 0.82 |
| drop | 1.17 | 88% | 2.28 | 98% | 0.78 / 0.83 |
| double | 1.59 | 88% | 2.35 | 95% | 0.93 / 0.91 |
| sub | 1.03 | 85% | 2.16 | 96% | 0.74 / 0.80 |
| unrelated | 1.15 | 79% | 2.42 | 100% | 0.88 / 0.88 |

Mean attention of the Wang et al. heads in each class (typo at S2):

| Head class (metric) | exact | swap | drop | double | sub | unrelated |
|---------------------|------|------|------|------|------|------|
| Duplicate token (S2→S1) | 0.52 | 0.03 | 0.05 | 0.09 | 0.03 | 0.02 |
| Induction (S2→after S1) | 0.40 | 0.05 | 0.07 | 0.11 | 0.05 | 0.07 |
| S-inhibition (END→S2) | 0.46 | 0.28 | 0.26 | 0.36 | 0.28 | 0.32 |
| Name mover (END→IO) | 0.55 | 0.26 | 0.27 | 0.32 | 0.26 | 0.25 |

![Head-class attention, typo at S2](results/typo_ioi_s2/300_examples/head_class_attention.png)

![Path patching by condition, typo at S2](results/typo_ioi_s2/300_examples/effects_by_condition.png)

**Takeaways**
- **GPT-2 small's IOI circuit does exact token matching, not fuzzy matching.** A single-character typo makes the model behave almost exactly as if the subject were a different person: logit diff and head attention both fall to the level of the `unrelated` control. The strongest duplicate token head, L3H0, drops from 0.80 attention on S1 to 0.02-0.09; induction head L5H5 drops from 0.88 to 0.05-0.16.
- **The failure starts at the first step of the circuit.** Duplicate token and induction heads no longer find S1, so the S-inhibition heads have nothing to suppress. Name movers split their attention between IO and S (END→IO ≈ 0.26 instead of 0.55), and the Negative Name Movers (L10H7, L11H10) lose almost all of their effect.
- **"double" typos (Susann) are the only partial exception.** They keep some duplicate-token attention (L3H0: 0.09 at S2, 0.20 at S1) and the most exact-like effect map (r = 0.93). This is plausibly because the typo'd word often still starts with a token shared with the original name.
- The ~85% IO>S that remains under typos matches the `unrelated` control, so it comes from positional/recency heuristics, not from recognising the name.
