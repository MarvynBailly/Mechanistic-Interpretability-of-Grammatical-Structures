...
@dataclass
class IOIConfig:
    device: str = None          # "cuda", "cpu", "mps", or None=auto
    n_examples: int = 2000
...
# Export for utils to use
import utils
utils.ONE_TOKEN_NAMES = ONE_TOKEN_NAMES
...
def run_ioi(config: IOIConfig):
    ...
    print("Loading model:", config.model_name, "on", device)
    ...
    if bad:
        raise ValueError(
            f"The following names are not single-token for {config.model_name}: {bad}"
        )

    # ---- Build IOI dataset ----
    print("\nBuilding IOI dataset...")
    dataset = build_dataset(config.n_examples)
    ...
    n_heat = min(config.n_heatmap_examples, len(dataset))
...
def main():
    # Default: auto device, original sizes
    config = IOIConfig()
    run_ioi(config)
